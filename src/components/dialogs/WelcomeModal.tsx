...
import {
  ...
  CheckCircle, AlertCircle, TrendingUp, Type, UserCircle2, Volume2, Mail
} from 'lucide-react'
import type {
...
export function WelcomeModal({ onComplete, initialPreferences, reassessmentMode, initialEmail }: WelcomeModalProps) {
  ...
  const [selectedManualLevel, setSelectedManualLevel] = useState<number | null>(null)
  ...
  const startConversation = useCallback(async () => {
    ...
    setIsAiTyping(true)
    try {
      const aiResponse = await geminiClient.generateOpener({ talkingoLevel: 1 }, userName)
      const aiText = aiResponse.aiResponse
      const aiTurn = { role: 'ai' as const, text: aiText }
      setTurns([aiTurn])
  ...
  }, [targetLanguage, nativeLanguage, userName])
  ...
  const sendTurn = useCallback(async (text: string) => {
    ...
    if (aiResponsesCount >= MAX_ONBOARDING_TURNS - 1) {
      ...
      setIsAiTyping(true)
      try {
        const aiResponse = await geminiClient.processUserMessage(trimmed, { talkingoLevel: 1 }, userName)
        const aiText = aiResponse.aiResponse
        const aiTurn = { role: 'ai' as const, text: aiText }
  ...
  }, [turns, isAiTyping, targetLanguage])

  // ── Analyze transcript and show results ─────────────────────────────────────
  const analyzeAndFinish = useCallback(async (_finalTurns: Array<{ role: 'user' | 'ai'; text: string }>) => {
    setStep('analyzing')
    geminiClient.stopSpeaking()
    try {
      const conversationText = _finalTurns.map(t => t.text).join(' ')
      const assessment = await geminiClient.assessLevel(conversationText, targetLanguage)
  ...
  const skipToLevel = useCallback((talkingoLevel: number) => {
    const level = talkingoLevelToLanguageLevel(talkingoLevel)
    const startingSeed = getStartingSeedForLevel(talkingoLevel)

    // Re-assessment (existing prefs present) should only change the LEVEL.
    // Preserve the learner's chosen tutor, current scenario, and every other
    // stored field (pathProgress, voice, uiLanguage…) by spreading their
    // existing preferences first. Brand-new onboarding has no prior prefs, so
    // it falls back to a sensible default tutor.
    const isReassessment = !!initialPreferences

    const prefs: UserPreferences = {
      // Carry over everything the modal doesn't explicitly manage.
      ...(initialPreferences ?? {}),
      // Level is the whole point of (re)assessment — always apply the new one,
      // keeping the derived language-level band in sync.
      talkingoLevel,
      level,
      // Setup fields — in re-assessment these equal the existing values (setup
      // is skipped), for new users they hold the freshly collected answers.
      targetLanguage,
      ...
      // Tutor / scenario: keep the learner's current choices on re-assessment;
      // use a sensible default for first-time setup.
      persona: isReassessment ? (initialPreferences?.persona ?? 'eli') : 'eli',
      currentUnitId: isReassessment ? (initialPreferences?.currentUnitId ?? startingSeed.id) : startingSeed.id,
    }
    finish(prefs)
  }, [targetLanguage, nativeLanguage, userName, preferredScript, learnerGender, initialPreferences])
    ...
    <div className="flex flex-col">
      ...
      <div className="px-6 sm:px-8 pb-6 sm:pb-8 border-t border-border/20 bg-card/95 pt-4">
        <button
          ...
          className="w-full py-3 px-4 rounded-lg bg-gradient-to-r from-primary to-primary-glow text-primary-foreground font-medium text-sm hover:shadow-lg hover:shadow-primary/25 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
  ...
  const renderConversation = () => {
    ...
    const currentQuestion = Math.min(Math.max(completedTurns, 1), MAX_ONBOARDING_TURNS)
    ...
    return (
    <div className="flex flex-col h-[min(520px,calc(100dvh-7rem))]">
      {/* Header */}
      ...
      {isListenStep && !isAiTyping && turns.length > 0 && (
        <div className="bg-primary/5 border-b border-primary/20 px-5 py-2 flex items-center gap-2">
          ...
          <p className="text-[11px] text-primary font-medium">Listen carefully and respond</p>
        </div>
      )}

      {/* Error message */}
      {assessmentError && (
        <div className="bg-error/10 border-b border-error/30 px-5 py-2">
          <p className="text-[11px] text-error">{assessmentError}</p>
        </div>
      )}
          ...
          <div className="flex flex-col items-center gap-3 py-2">
            <button
              ...
              className={cn(
                ...
                isListening
                  ...
                  : 'bg-gradient-to-br from-primary to-primary-glow border-primary/50 text-primary-foreground hover:scale-105 shadow-lg shadow-primary/25'
              )}
            ...
            {userInput.trim() && !isListening && (
              <button
                ...
                disabled={isAiTyping}
                className="px-4 py-2 rounded-full bg-primary text-primary-foreground text-xs font-medium hover:bg-primary/90 disabled:opacity-40"
              >
                Send &ldquo;{userInput.length > 20 ? userInput.slice(0, 20) + '…' : userInput}&rdquo;
              </button>
            )}
  ...
  const LEVEL_OPTIONS: Array<{ level: number; label: string; description: string }> = [
    { level: 1, label: 'First Words', description: 'I\'m just starting — teach me from zero' },
    { level: 2, label: 'Building Blocks', description: 'I know some words and basic phrases' },
    { level: 3, label: 'Survival Mode', description: 'I can handle simple real-life situations' },
    { level: 4, label: 'Daily Explorer', description: 'I can get by in everyday situations' },
    { level: 5, label: 'Conversation Ready', description: 'I can hold a basic conversation' },
    { level: 6, label: 'Confident Talker', description: 'I can express my thoughts clearly' },
    { level: 7, label: 'Confident Speaker', description: 'I speak fairly fluently with some gaps' },
    { level: 8, label: 'Advanced Talker', description: 'I can discuss abstract topics' },
    { level: 9, label: 'Almost Native', description: 'I\'m comfortable in most situations' },
    { level: 10, label: 'Expert Speaker', description: 'I speak with nuance and precision' },
    { level: 11, label: 'Precision Speaker', description: 'I master subtle language distinctions' },
    { level: 12, label: 'Mastery', description: 'I speak nearly like a native' },
  ]
    ...
    return (
      <div className="p-6 sm:p-8">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-muted-foreground">
            {hasError ? 'We couldn\'t complete the assessment' : `Level ${result?.talkingoLevel || 1} · ${getLevelByNumber(result?.talkingoLevel || 1).name}`}
          </p>
        </div>
        ...
        {hasError && (
          <div className="bg-error/10 border border-error/30 rounded-xl p-4 mb-6">
            <p className="text-sm text-error mb-3">{assessmentError}</p>
            <div className="flex flex-col gap-2">
              <button
                onClick={retryAssessment}
                className="w-full py-2 px-4 rounded-lg bg-primary text-primary-foreground text-sm hover:bg-primary/90 transition-colors"
              >
                Retry Assessment
              </button>
              <button
        ...
        {!hasError && (
          <div className="flex gap-3">
            ...
            <button
              onClick={() => result && skipToLevel(result.talkingoLevel)}
              className="flex-1 py-3 px-4 rounded-lg bg-gradient-to-r from-primary to-primary-glow text-primary-foreground font-medium text-sm hover:shadow-lg hover:shadow-primary/25 transition-all flex items-center justify-center gap-2"
            >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true" aria-label="Set up your tutor">
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 w-32 h-32 bg-gradient-to-tr from-primary/[0.07] to-transparent rounded-full blur-3xl pointer-events-none" />
