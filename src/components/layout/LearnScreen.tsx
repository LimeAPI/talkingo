...
import {
  ...
  ChevronDown, Circle,
  GraduationCap, Hash, Lock, ArrowUp, Play,
} from 'lucide-react'
...
import { getCompletedLessons, getCompletedLessonsWithQuality, getLessonStatusMap } from '@/lib/storage/lesson-progress'
import type { LessonQuality, CompletedLessonsRecord, LessonStatus } from '@/lib/storage/lesson-progress'
import type {
...
interface LearnScreenProps {
  ...
  onReassess?: () => void
  /** Advance the learner to a new level once they complete their current one. */
...
// Level identity is communicated by tier depth (opacity/weight) on the gold
// brand — not by competing hues. Phase tiers deepen as the learner progresses.
const LEVEL_COLORS: Record<number, string> = {
  1: 'bg-primary/8 text-primary border-primary/20',
  2: 'bg-primary/8 text-primary border-primary/20',
  3: 'bg-primary/10 text-primary border-primary/25',
  4: 'bg-primary/12 text-primary border-primary/25',
  5: 'bg-primary/12 text-primary border-primary/30',
  6: 'bg-primary/14 text-primary border-primary/30',
  7: 'bg-primary/16 text-primary border-primary/35',
  8: 'bg-primary/16 text-primary border-primary/35',
  9: 'bg-primary/18 text-primary border-primary/40',
  10: 'bg-primary/20 text-primary border-primary/40',
  11: 'bg-primary/22 text-primary border-primary/45',
  12: 'bg-primary/24 text-primary border-primary/50',
}
...
function PhaseBasedJourney({
  ...
  /** The learner's current level — levels above this are locked until they advance. */
  userLevel: number
}) {
  ...
  const getPhaseMeta = (phase: string) => {
    switch (phase) {
      case 'Foundation':
        return { tint: 'bg-primary/8 border-primary/20 text-primary', icon: GraduationCap }
      case 'Building':
        return { tint: 'bg-primary/12 border-primary/25 text-primary', icon: Zap }
      case 'Fluency':
        return { tint: 'bg-primary/16 border-primary/30 text-primary', icon: Sparkles }
      case 'Mastery':
        return { tint: 'bg-primary/22 border-primary/40 text-primary', icon: TrendingUp }
      default:
          ...
          >
            ...
            <div className="px-2 pb-2 space-y-2">
              {levels.map(({ level, filtered, completed, total }) => {
                ...
                // Levels above the learner's current level are locked — they must
                // complete their current level and tap "Level Up" to unlock them.
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <span className="flex-shrink-0 mt-0.5 text-primary">
                          {isLocked ? <Lock className="w-4 h-4 text-foreground/40" /> : <GraduationCap className="w-4 h-4" />}
                        </span>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {isLocked && (
                              <span className="text-[9px] px-1.5 py-0.5 rounded-full border border-border/40 text-foreground/50 bg-card font-semibold">
                                Locked
                              </span>
                            )}
                          </div>
                      ...
                      {isExpanded && (
                        ...
                        >
                          <div className="px-4 pb-4 pt-2 border-t border-border/20 rounded-b-lg">
                            {isLocked ? (
                              <div className="flex items-center gap-2.5 px-3 py-3 rounded-lg bg-card/60 border border-border/30">
                                <Lock className="w-4 h-4 text-foreground/40 flex-shrink-0" />
                                <p className="text-[11px] text-foreground/60 leading-snug">
                                  Complete Level {userLevel} and level up to unlock these scenarios.
                                </p>
                              </div>
                            ) : (
                            <>
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <div className="text-[10px] text-foreground/40 flex items-center gap-1.5">
                                {(() => {
                                  const practicing = filtered.filter((s) => {
                                    const st = lessonStatus[s.id] ?? 'new'
                                    return st === 'practicing' && !completedLessons.includes(s.id)
                                  }).length
                                  return practicing > 0 ? (
                                    <span className="text-primary font-semibold">{practicing} in progress</span>
                                  ) : null
                                })()}
                            ...
                            <div className="space-y-1.5 mt-1">
                              {(() => {
                                ...
                                return filtered.map((scenario: CachedScenario, idx: number) => {
                                  const status = lessonStatus[scenario.id] ?? 'new'
                                  const isDone = status === 'done' || completedLessons.includes(scenario.id)
                                  const isPracticing = !isDone && status === 'practicing'
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={cn(
                                        'w-full text-left flex items-center gap-2.5 px-3 py-2.5 rounded-lg border transition-all hover:shadow-[0_0_15px_-6px_oklch(var(--primary)/0.06)]',
                                        isDone
                                          ? 'bg-success/5 border-success/15 hover:border-success/30'
                                          : isNext
                                            ? 'bg-primary/5 border-primary/50 shadow-[0_0_0_1px_oklch(var(--primary)/0.20),0_0_18px_-10px_oklch(var(--primary)/0.40)] hover:border-primary/70'
                                            : isPracticing
                                              ? 'bg-primary/[0.03] border-primary/30 hover:border-primary/50'
                                    ...
                                    >
                                      ...
                                      {isDone ? (
                                        (() => {
                                          ...
                                          // Few corrections = clean (success); more = needs work.
                                        ...
                                        // Half-filled ring — "you started this, keep going"
                                        <svg className="w-4 h-4 -rotate-90 flex-shrink-0 text-primary" viewBox="0 0 36 36">
                                          <circle cx="18" cy="18" r="15" fill="none" stroke="currentColor" strokeWidth="4" className="text-primary/20" />
                                          <circle
                                            cx="18" cy="18" r="15" fill="none" stroke="currentColor" strokeWidth="4"
                                            strokeDasharray="56 94.2" strokeLinecap="round"
                                          />
                                        </svg>
                                      ) : (
                                      ...
                                      {isPracticing && (
                                        <span className="text-[8px] px-1.5 py-0.5 rounded-full border border-primary/30 bg-primary/10 text-primary font-bold flex-shrink-0">
                                          Keep going
                                        </span>
                                      )}
...
export function LearnScreen({ preferences, progress, userId, onStartSession, onReassess, onLevelUp }: LearnScreenProps) {
  ...
  // ── Level completion (independent of search/grammar filters) ──────────────
  // Drives manual progression: the learner advances by completing their current
  // level's scenarios, then taps "Level Up" to unlock the next one.
  const levelCompletion = useMemo(() => {
    const map = new Map<number, { completed: number; total: number }>()
    const done = new Set(completedLessons)
    for (const s of scenarios) {
      if (s.id === 'free-talk') continue
      const lvl = parseInt(String(s.difficulty), 10) || 1
      const entry = map.get(lvl) ?? { completed: 0, total: 0 }
      entry.total += 1
      if (done.has(s.id)) entry.completed += 1
      map.set(lvl, entry)
    }
    return map
  }, [scenarios, completedLessons])

  const currentLevelStats = levelCompletion.get(userLevel) ?? { completed: 0, total: 0 }
  const currentLevelComplete = currentLevelStats.total > 0 && currentLevelStats.completed >= currentLevelStats.total
  const canLevelUp = !!onLevelUp && userLevel < 12 && currentLevelComplete
  const remainingInLevel = Math.max(0, currentLevelStats.total - currentLevelStats.completed)

  // ── Next actionable scenario in the current level ──────────────────────────
  // Powers the "Continue" hero: resume a practicing scenario first, else the
  // first not-done one. Null when the level is complete (the level-up banner
  // takes over).
  const nextScenario = useMemo(() => {
    const inLevel = scenarios.filter(
      (s) => s.id !== 'free-talk' && (parseInt(String(s.difficulty), 10) || 1) === userLevel
    )
    const done = new Set(completedLessons)
    const resume = inLevel.find((s) => (lessonStatus[s.id] ?? 'new') === 'practicing' && !done.has(s.id))
    if (resume) return { scenario: resume, resume: true }
    const fresh = inLevel.find((s) => !done.has(s.id))
    if (fresh) return { scenario: fresh, resume: false }
    return null
  }, [scenarios, userLevel, lessonStatus, completedLessons])
    ...
    <div className="relative flex-1 min-h-0 flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto [&::-webkit-scrollbar]:hidden px-4 sm:px-6">
        <div className="max-w-lg mx-auto w-full space-y-4 pb-24">
          ...
          {/* ── Continue hero: one tap to the next conversation ── */}
          {nextScenario && (
            <motion.button
              initial={{ opacity: 0, y: -6 }}
              animate={{ opacity: 1, y: 0 }}
              onClick={() => onStartSession(nextScenario.scenario.id, 'new')}
              className="w-full text-left surface-card--elevated p-4 hover:border-primary/40 transition-all group"
            >
              <div className="flex items-center gap-3">
                <span className="w-11 h-11 rounded-xl bg-primary/15 border border-primary/30 flex items-center justify-center text-primary flex-shrink-0">
                ...
                <ArrowUp className="w-4 h-4 text-primary rotate-90 flex-shrink-0 group-hover:translate-x-0.5 transition-transform" />
              </div>
            </motion.button>
          )}

          {/* ── Level-up banner: complete the current level to advance ── */}
          {canLevelUp && (
            <motion.div
              initial={{ opacity: 0, y: -6 }}
              animate={{ opacity: 1, y: 0 }}
              className="surface-card--elevated p-4"
            >
              <div className="flex items-center gap-3">
                <span className="w-10 h-10 rounded-xl bg-primary/15 border border-primary/30 flex items-center justify-center text-primary flex-shrink-0">
                  <TrendingUp className="w-5 h-5" />
                </span>
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-bold text-foreground">
                    Level {userLevel} complete
                  </h3>
                  <p className="text-[11px] text-foreground/60 mt-0.5">
                    You finished every scenario. Ready for Level {userLevel + 1} — {LEVEL_LABELS[userLevel + 1] ?? `Level ${userLevel + 1}`}?
                  </p>
                </div>
              </div>
              <button
                onClick={() => onLevelUp?.(userLevel + 1)}
                className="btn-gradient w-full justify-center mt-3 text-xs"
              >
                <ArrowUp className="w-3.5 h-3.5" />
                Level up to Level {userLevel + 1}
              </button>
            </motion.div>
          )}

          {/* ── Progress hint toward the next level ── */}
          {!canLevelUp && userLevel < 12 && currentLevelStats.total > 0 && remainingInLevel > 0 && (
            <div className="rounded-xl border border-border/40 bg-card/60 px-3.5 py-2.5 flex items-center gap-2.5">
              <span className="w-7 h-7 rounded-lg bg-primary/10 border border-primary/20 flex items-center justify-center text-primary flex-shrink-0">
                <GraduationCap className="w-3.5 h-3.5" />
              </span>
              <p className="text-[11px] text-foreground/70 leading-snug">
                Complete <span className="font-bold text-foreground">{remainingInLevel}</span> more
                {remainingInLevel === 1 ? ' scenario' : ' scenarios'} in Level {userLevel} to unlock Level {userLevel + 1}.
              </p>
            </div>
          )}
          ...
          <div className="mb-2 space-y-2.5">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl border border-border/40 bg-card text-sm focus:outline-none focus:border-primary/40 focus:shadow-[0_0_20px_-6px_oklch(var(--primary)/0.12),0_0_8px_-2px_oklch(var(--primary)/0.08)] placeholder:text-foreground/40 shadow-sm transition-all duration-200"
              />
            </div>
          ...
          {scenariosLoading && (
            <div className="flex items-center justify-center py-16 text-foreground/60 gap-2.5">
              ...
              <span className="text-sm font-medium">Loading scenarios&hellip;</span>
            </div>
          )}
