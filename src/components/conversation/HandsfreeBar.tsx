...
export function HandsfreeBar({
  ...
  // Report the bar's height so the transcript pads itself exactly.
  useEffect(() => {
  ...
  const stateColor = isProcessing
    ? 'text-amber-400'
    : isSpeaking
    ? (isNative ? 'text-amber-400' : 'text-primary')
    : isListening
  ...
  const orbBorder = isProcessing
    ...
    : isSpeaking
    ? (isNative ? 'border-amber-400/50 shadow-amber-400/20' : 'border-primary/50 shadow-primary/20')
    : isListening
    ? (isNative ? 'border-amber-500/50 shadow-amber-500/20' : 'border-primary/50 shadow-primary/20')
    : 'border-border/40'

  const barBorder = isSpeaking
    ? (isNative ? 'border-amber-400/40 shadow-amber-400/10' : 'border-primary/40 shadow-primary/10')
    : isListening
            ...
            >
              ...
              <div className="flex items-center justify-center gap-2 pt-3 pb-1.5">
                ...
                <span className={cn(
                  ...
                  isProcessing ? 'bg-amber-400' : isSpeaking ? (isNative ? 'bg-amber-400' : 'bg-primary') : isListening ? (isNative ? 'bg-amber-500' : 'bg-primary') : 'bg-transparent',
              ...
              <div className="flex items-center justify-center gap-3 px-4 pb-4 pt-1.5">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    voiceNotesEnabled
                      ...
                      : 'bg-card/80 border-border/50 text-muted-foreground/60 hover:border-primary/40 hover:text-primary/80'
                  )}
                >
                ...
                >
                  ...
                  {isProcessing
                    ...
                    : isSpeaking
                    ? <Volume2 className={cn('w-6 h-6 animate-pulse', isNative ? 'text-amber-400' : 'text-primary')} />
                    : isListening
