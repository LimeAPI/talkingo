...
import { getDodoTrialProductId } from '@/lib/dodopayments/env'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Trial capability ($5 / 5-day → auto-converts to monthly). Stripe's trial
    // price is a REQUIRED env var, so a Stripe trial is always available when
    // Stripe is enabled. Dodo's trial needs the optional one-time trial product;
    // without it we must NOT offer the trial (it would create a $0 trial that
    // contradicts the "$5" promise), so the picker hides the trial for Dodo.
    const stripeTrial = stripeEnabled
    const dodoTrial = dodoEnabled && !!getDodoTrialProductId()
...
export async function GET() {
  ...
  // fabricate a recommendation. Return the same validation error a POST with no
  // codes would produce (Req 4.2) rather than constructing a fake request.
  return NextResponse.json(
    {
