...
export async function POST(req: NextRequest) {
  ...
  // Block plan changes during a trial: a switch prorates immediately at the
  // provider, which would end the trial and charge right away. The UI hides the
  // switch during a trial; this is the server-side backstop.
  if (stored.status === 'trialing') {
    return NextResponse.json(
      {
        error: 'not_allowed_during_trial',
        message: 'You can switch plans once your trial converts to a paid plan.',
      },
      { status: 409 },
    )
  }
