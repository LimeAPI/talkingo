...
const ttsSchema = z.object({
  ...
  provider: z.enum(['auto', 'edge', 'gemini']).optional(),
}).passthrough()
...
export async function POST(req: NextRequest) {
  ...
  if (provider === 'gemini') {
    // The forced-Gemini path is the deliberate PAID path (premium voice
    // previews). Free Edge TTS stays open to everyone below; this branch is
    // gated to active/trialing subscribers. Fails CLOSED on any read error.
    try {
