...
const RESPONSE_FORMAT = `Return ONLY valid JSON (no markdown, no code fences):
{
  ...
  "keyWords": ["useful target-language word or short phrase from your reply"] | null,
...
- "keyWords": Up to 3 words or short phrases, written in the language the learner is studying (never English labels), that you used in your reply and that are worth the learner practicing. null if none.
- "unitComplete": Set true ONLY when, within THIS conversation, the learner has clearly handled what this scenario is about — they did the kind of speaking it calls for, at their level, without needing heavy help. Judge the scenario, not perfection: a few small slips are fine. Most turns this is false; set it true once they've genuinely shown it (and keep it true afterwards). If there's no specific scenario (free chat), always false.`
...
/**
 * Transient adaptive nudge. When the learner has been making a lot of mistakes
 * recently, tell the AI to ease up for this reply — slow down, simplify, lean
 * on the native language a touch more, and rebuild confidence. Empty otherwise.
 */
function buildAdaptiveBlock(state?: ConversationState): string {
  if (state?._adaptiveHint !== 'high-error-rate') return ''
  return `ADAPTIVE NOTE — The learner has been struggling this session (lots of slips). For your next reply: slow down, use shorter and simpler sentences, ask one easy question, and gently rebuild their confidence. Don't pile on corrections — fix at most one thing by example. Keep it warm and encouraging.`
}

/**
 * Transient teaching nudge from the live Session Coach. Tells the AI to weave in
 * ONE unused target word, or to model a just-corrected form again — for the next
 * reply only. The coach guarantees this never fires while the learner is
 * struggling, and the instruction itself forbids announcing or quizzing, so the
 * conversation stays natural ("never drill"). Empty when there's no nudge.
 */
function buildCoachBlock(state?: ConversationState): string {
  const nudge = state?._coachNudge?.trim()
  if (!nudge) return ''
  return `TEACHING NUDGE (next reply only — weave it in invisibly, never announce or quiz): ${nudge}`
}
...
/**
 * Grammatical gender agreement. Languages like Spanish, French, Arabic, Hindi,
 * etc. change adjective / participle / pronoun forms based on the gender of the
 * person being addressed. When the learner has told us how they'd like to be
 * referred to, pass it through so the AI conjugates correctly ABOUT them.
 * Empty when unset or for languages without grammatical gender.
 */
function buildGenderBlock(state?: ConversationState): string {
  if (!state?.learnerGender) return ''
  const form = state.learnerGender === 'masculine' ? 'masculine' : 'feminine'
  return `LEARNER GENDER — When grammar requires gender agreement for the person you are speaking TO (adjectives, past participles, pronouns, and honorifics that refer to the learner), use ${form} forms. This applies only to references to the learner themselves — not to other people or objects.`
}
...
export function getSystemInstruction(state?: ConversationState): string {
  ...
  const blocks = [
    ...
    // 4c. Gender — grammatical gender agreement for the learner (gendered langs)
    buildGenderBlock(state),
    // 5. Scenario — conversational backdrop
    buildScenarioBlock(state),
    // 6. Memory — what you know about them
    buildMemoryBlock(state),
    // 6b. Adaptive — transient ease-up when the learner is struggling
    buildAdaptiveBlock(state),
    // 6c. Teaching nudge — transient "circle back" from the live Session Coach
    buildCoachBlock(state),
...
export function buildOpenerPrompt(state: ConversationState, userName?: string): string {
  ...
  // Keep the greeting on-level and short enough to work as speech AND text.
  // The level's own aiBehavior governs tone/complexity; we just remind the model
  // to honor it from the very first line and keep the opener tight.
  const levelCue = ` Match Level ${level}/12 ("${levelData.name}") — ${levelData.aiBehavior} Keep the opener to 1–2 short sentences and end with a simple question.`

  if (state.customPrompt) {
    return `Start a fresh conversation.${nameStr} The user wants to talk about: ${state.customPrompt}. Open naturally — be warm, be yourself, and end with something that makes them want to respond.${memoryClause}${levelCue}${langAnchor}`
  }
  ...
  if (seed) {
    // Returning to a scenario they've already worked on: open like you're
    // picking the topic back up together, not starting from zero.
    const resumeClause = state._resumeScenario
      ? ` The learner is coming back to this scenario to keep practicing it — greet them like you're picking it up together again, not meeting fresh. Don't recap what they did before; just warmly re-open the situation.`
      : ''
    return `Start a fresh conversation.${nameStr} The setting is: ${seed.scenarioBrief}.${resumeClause} Open naturally — be warm, be yourself, and end with something that makes them want to respond.${memoryClause}${levelCue}${langAnchor}`
  }

  return `Start a fresh conversation.${nameStr} Ask what they'd like to talk about or suggest something light. Be warm, be yourself.${memoryClause}${levelCue}${langAnchor}`
}
