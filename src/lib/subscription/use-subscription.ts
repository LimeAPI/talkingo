...
/**
 * Whether the user has a subscription that should be MANAGED (shown the
 * SubscriptionManager) rather than offered a fresh upgrade.
 *
 * Covers `active`, `trialing`, AND `past_due` — a past-due user had a real
 * subscription and needs the "update payment" recovery path, NOT a brand-new
 * checkout (starting a second subscription would double-charge them). Cleanly
 * lapsed states (`expired` / `canceled` / `none`) are intentionally excluded:
 * those flow through the normal upgrade/resubscribe path.
 */
export function hasManageableSubscription(userId?: string | null): boolean {
  const info = getSubscriptionInfo(userId)
  return (
    info.status === 'active' || info.status === 'trialing' || info.status === 'past_due'
  )
}
