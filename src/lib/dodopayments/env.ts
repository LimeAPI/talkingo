...
/**
 * Optional one-time product id for the 5-day trial fee ($5). Bundled with the
 * first subscription payment via `one_time_product_cart` so the Dodo trial
 * mirrors Stripe (one-time $5 + monthly + 5-day trial). NOT part of the
 * required-credentials set — when unset, the trial simply has no upfront fee
 * (a plain free trial) rather than disabling the provider. Accepts both
 * `DODOPAYMENTS_PRODUCT_TRIAL` and `DODO_PAYMENTS_PRODUCT_TRIAL` spellings.
 */
export function getDodoTrialProductId(): string | undefined {
  const raw =
    process.env.DODOPAYMENTS_PRODUCT_TRIAL ?? process.env.DODO_PAYMENTS_PRODUCT_TRIAL
