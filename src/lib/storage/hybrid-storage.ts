  ...
  try {
    ...
    if (prefs) {
      try {
        ...
        return !!(parsed.targetLanguage && (parsed.level || parsed.talkingoLevel))
      } catch {
        return false
      }
    }
 ...
 *
 * Robustness: the cross-device winner is the copy whose MOST RECENT session is
 * newest (recency wins), not simply the one with more sessions — a device that
 * just finished a session has the freshest data even if it has fewer total
 * sessions (both are capped at 12). Session count is only a tie-breaker. The
 * user note prefers a non-empty / longer value so a stray empty sync can't wipe it.
 */
export async function loadAndMergeStructuredMemory(
  ...
  // Recency is measured over REAL sessions only — a migration stub (scenarioId
  // 'migration') must never win, or a clobbered/legacy remote could overwrite a
  // device that has genuine session history.
  const latestRealDate = (m: StructuredMemory) =>
    m.sessions.reduce(
      (max, s) => (s.scenarioId === 'migration' ? max : Math.max(max, s.date || 0)),
      0,
    )

  const lDate = latestRealDate(local)
  const rDate = latestRealDate(remote)

  let winner: StructuredMemory
  if (rDate !== lDate) {
    winner = rDate > lDate ? remote : local
  } else {
    // Same recency (or both have no real sessions) — keep the more complete copy.
    winner = remote.sessions.length >= local.sessions.length ? remote : local
  }

  // Never let an empty note clobber a real one; prefer the longer of the two.
  const notes = [local.userNote, remote.userNote].filter((n) => !!n && n.trim().length > 0)
  if (notes.length > 0) {
    winner.userNote = notes.sort((a, b) => b.length - a.length)[0]
  }
