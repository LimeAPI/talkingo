...
export function WelcomeModal({ onComplete, initialPreferences, reassessmentMode, initialEmail }: WelcomeModalProps) {
  ...
  const startConversation = useCallback(async () => {
    ...
    setIsAiTyping(true)
    try {
      const aiResponse = await geminiClient.generateOpener({ talkingoLevel: 1, topic: learningGoal, correctionStyle: 'silent' }, userName)
  ...
  const sendTurn = useCallback(async (text: string) => {
    ...
    if (aiResponsesCount >= MAX_ONBOARDING_TURNS - 1) {
      ...
      setIsAiTyping(true)
      try {
        const aiResponse = await geminiClient.processUserMessage(trimmed, { talkingoLevel: 1, topic: learningGoal ?? 'general', correctionStyle: 'silent' }, userName)
        const aiText = aiResponse.aiResponse
        const aiTurn = { role: 'ai' as const, text: aiText }
