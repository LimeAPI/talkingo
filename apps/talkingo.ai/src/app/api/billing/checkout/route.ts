...
export async function POST(req: NextRequest) {
  ...
  const { provider, plan, method, email } = body
  ...
  // The 5-day trial is a Stripe-only construct ($1 one-time fee + Stripe's
  // native trial_period_days). DodoPayments has no equivalent, and its checkout
  // would silently bill the full monthly price with no trial — so reject the
  // combination outright rather than mischarge the customer.
  if (provider === 'dodopayments' && plan === 'trial') {
    return NextResponse.json(
      {
        error: 'invalid_plan',
        message: 'The free trial is only available with card via Stripe. Pick monthly or yearly to pay with Dodo.',
      },
      { status: 400 },
    )
  }
