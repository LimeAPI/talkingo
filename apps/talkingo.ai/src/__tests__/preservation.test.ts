...
// Observation: The MASTER_PROMPT already instructs the AI to "Correct naturally,
// not constantly" and to model correct language in replies rather than drilling.
// This natural style guidance exists in the prompt system.
    ...
    fc.assert(
      fc.property(
        ...
        (prompt) => {
          ...
          // The existing prompt instructs natural corrections
          expect(lowerPrompt).toContain('correct naturally')

          // The existing prompt discourages drill-like behavior
          expect(lowerPrompt).toContain('avoid drills')

          // The prompt has a positive example of natural correction
          // "Oh nice — you went to a restaurant yesterday?"
          expect(prompt).toContain('went to a restaurant')
        }
      ),
      { numRuns: 1 }
    )
