...
const correctionStyleArb = fc.constantFrom<CorrectionStyle>('direct', 'silent')

const personaIdArb = fc.constantFrom<PersonaId>('eli', 'alex', 'dr-luma', 'sofia', 'riko', 'marco')

const learningGoalArb = fc.constantFrom<LearningGoal>('travel', 'career', 'daily-life', 'academic', 'cultural')
...
const topicArb = fc.constantFrom<ConversationTopic>(
...
const userPreferencesArb: fc.Arbitrary<UserPreferences> = fc.record({
  ...
  topic: topicArb,
  correctionStyle: correctionStyleArb,
  ...
  learningGoal: fc.option(learningGoalArb, { nil: undefined }),
    ...
    fc.assert(
      fc.property(
        ...
        (originalPrefs, newDialect) => {
          ...
          expect(updated.topic).toBe(originalPrefs.topic)
          expect(updated.correctionStyle).toBe(originalPrefs.correctionStyle)
          ...
          expect(updated.learningGoal).toBe(originalPrefs.learningGoal)
      ...
      fc.property(
        ...
        (originalPrefs, field, value) => {
          ...
          const allFields: (keyof UserPreferences)[] = [
            'talkingoLevel', 'level', 'topic', 'correctionStyle', 'persona',
            'userName', 'targetLanguage', 'nativeLanguage', 'learningGoal',
