...
import { getSeedById } from '@talkingo/shared/curriculum'
...
export function loadStructuredMemory(userId: string | null): StructuredMemory {
  try {
    ...
    if (parsed?.version === 2) return parsed as StructuredMemory
...
export function processSessionEnd(
  ...
  const seed = scenarioId && scenarioId !== 'free-talk' && !scenarioId.startsWith('custom-')
    ...
    : null
  const newVocab = processVocabulary(memory, messages, seed ?? null, scenarioId)
...
function processVocabulary(
  ...
  // Step 1: Mark seed vocabulary as "introduced" if not already tracked
  if (seed?.targetVocab) {
    for (const word of seed.targetVocab) {
  ...
  // Step 2: Scan user messages for vocabulary production
...
/**
 * Smart word detection — handles partial matches for inflected languages.
 * Checks if user text contains the target word (allowing for conjugation variance).
 */
function textContainsWord(text: string, word: string): boolean {
  // Direct inclusion (handles phrases and conjugated forms)
  if (text.includes(word)) return true

  // For single words, check word boundary (3+ chars to avoid false positives)
  if (word.length >= 3) {
    ...
    if (raw.startsWith('{')) {
      try {
        ...
        if (parsed?.version === 2) {
          ...
          return parsed as StructuredMemory
        }
      } catch {
