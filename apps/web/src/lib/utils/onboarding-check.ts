 ...
 * 2. Essential data validation (targetLanguage + level + learningGoal)
...
export function shouldSkipOnboarding(
  ...
  // - level OR cefr (their proficiency level)
  // - learningGoal (why they're learning)
  
  const hasTargetLanguage = !!prefs.targetLanguage
  const hasLevel = !!(prefs.level || prefs.cefr)
  const hasLearningGoal = !!prefs.learningGoal
  
  const hasEssentialData = hasTargetLanguage && hasLevel && hasLearningGoal

  if (hasEssentialData) {
    console.log('[Onboarding] Essential data present, skipping onboarding', {
      targetLanguage: prefs.targetLanguage,
      level: prefs.level || prefs.cefr,
  ...
  if (hasTargetLanguage || hasLevel || hasLearningGoal) {
...
export function validatePreferences(prefs: UserPreferences): {
  ...
  if (!prefs.level && !prefs.cefr) missingFields.push('level or cefr')
  if (!prefs.learningGoal) missingFields.push('learningGoal')
